    ...
    public class ExhibitRepository : IExhibitRepository
    {
        ...
        private IQueryable<Exhibit> ApplyFiltering(IQueryable<Exhibit> query, ExhibitFilter filter)
        {
            ...
            if (filter.CreationExactDate.HasValue ||
                filter.CreationYear.HasValue ||
                filter.CreationCentury.HasValue)
            {
                query = query.Where(e =>
                    (filter.CreationExactDate.HasValue && e.CreationExactDate == filter.CreationExactDate) ||
                    (filter.CreationYear.HasValue && e.CreationYear == filter.CreationYear) ||
                    (filter.CreationCentury.HasValue && e.CreationCentury == filter.CreationCentury)
                );
            }
                ...
                case "creation":
                    query = filter.Descending
                        ? query.OrderByDescending(e =>
                              e.CreationExactDate.HasValue
                                  ? e.CreationExactDate.Value
                                  : e.CreationYear.HasValue
                                      ? new DateOnly(e.CreationYear.Value, 1, 1)
                                      : e.CreationCentury.HasValue
                                          ? new DateOnly((e.CreationCentury.Value - 1) * 100 + 1, 1, 1)
                                          : DateOnly.MaxValue)
                        : query.OrderBy(e =>
                              e.CreationExactDate.HasValue
                                  ? e.CreationExactDate.Value
